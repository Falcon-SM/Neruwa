...
- 就寝前の点字学習、睡眠中の読み上げ、睡眠セッション単位の朝の確認テスト
- 学習カード・再生設定・テスト結果のアカウント別ローカル保存
- 最新の睡眠時間・目標達成率・任意の気分・短文をアプリ内フィードへ投稿
- 匿名表示名のタイムラインと「おやすみ」リアクション（ゲスト・オフライン時はローカル表示）
- 必要な場合に限り、同じ睡眠サマリーをiOS共有シートで外部共有
- 19:00〜翌04:59の静的な星空と、それ以外の時間帯の静的な朝・昼の空背景

TMTは現在のiOS版には含めず、朝の流れではスキップします。共有フィードは認証済みユーザーで検証するMVPです。招待した友達だけに公開する仕組み、ブロック、通報、公開範囲は本番公開前に追加します。
...
- 19:00〜翌04:59: `日記 → 点字学習 → 音声設定 → 睡眠記録`を必須表示し、星空の背景にする
- 05:00〜18:59: `気分 → 点字テスト → 記録`を必須表示し、朝・昼の空背景にする

朝の確認テストは必須フロー中に通常スキップできません。カードが2枚未満でテストを構成できない場合だけ、理由を表示して記録へ進めます。朝に睡眠セッションがまだない場合も気分を日次進捗へ保持するため、初回利用で行き止まりにはなりません。夜は睡眠タイマーの開始、手入力、またはHealthKitからの保存で完了します。
...
公開投稿には匿名表示名、起床日、睡眠時間、目標達成率、任意の気分、80文字以内の投稿文だけを持たせます。所有者UIDは本人だけが読める別ドキュメントへ分離します。正確な就寝・起床時刻、睡眠ステージ、HealthKit識別情報、夜の日記、朝の振り返り本文は公開投稿にフィールド自体を用意していません。
