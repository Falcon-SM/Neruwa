...
「夜に睡眠を記録し、朝の状態と一緒に振り返る」ための SwiftUI アプリです。Web PoC の中心フローと夜空の配色を引き継ぎつつ、iOS 26 の Liquid Glass、HealthKit、Firebase を使うネイティブ構成にしています。
...
- 長い履歴とカード一覧は `LazyVStack` で必要な部分だけ生成
- 繰り返し表示するカードは静的な `SurfaceCard`、Liquid Glass は主要カードと操作に限定
- 睡眠時計の1秒更新は、計測中かつ画面が表示され、アプリが前面にある間だけ実行
- 夜空の背景と装飾線をタッチ判定から外し、各画面の縦スクロールを1つに統一
